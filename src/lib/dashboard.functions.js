...
export const getExecutiveDashboardData = createServerFn({ method: "GET" })
    .validator((data) => dashboardFilterSchema.parse(data))
    .handler(async ({ data }) => {
    ...
    const getRevenue = async (s, e) => {
        let query = supabase
            ...
            .lte("receipt_date", e);
        if (insurerId) {
            const { data: commIds } = await supabase.from("commissions").select("id").eq("insurer_id", insurerId);
            if (commIds?.length) {
                query = query.in("commission_id", commIds.map((c) => c.id));
            }
        }
    ...
    const getRenewalCount = async (days) => {
    ...
    const oppsByStatus = (opportunities || []).reduce((acc, curr) => {
        if (curr.status) {
            acc[curr.status] = (acc[curr.status] || 0) + 1;
        }
        return acc;
    }, {});
    const crossSellCount = (opportunities || []).filter((o) => o.status === 'cross_sell').length;
    ...
    const ranking = (insurerRankingData || []).reduce((acc, curr) => {
    ...
    return {
        finance: {
            ...
            overduePayables,
            totalBalance,
            bankAccounts: (accounts || []).map((a) => ({ name: a.name, balance: Number(a.balance) })),
        },
        operation: {
