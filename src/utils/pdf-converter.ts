...
export async function convertPdfToImages(file: File, maxPages: number = 3): Promise<{ base64: string; mimeType: string }[]> {
  ...
  for (let i = 1; i <= pagesToProcess; i++) {
    ...
    await page.render({
      ...
      viewport: viewport,
      // @ts-ignore - Some versions of pdfjs-dist expect 'canvas' property which is the canvas element itself
      canvas: canvas
    }).promise;
    ...
    images.forEach((imgData, index) => {
      ...
      img.onload = () => {
        ...
        if (loadedCount === images.length) {
          const totalHeight = loadedImages.reduce((sum, img) => sum + (img?.height || 0), 0);
          const maxWidth = Math.max(...loadedImages.map(img => img?.width || 0));
