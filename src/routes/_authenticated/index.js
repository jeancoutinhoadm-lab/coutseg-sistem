...
import { TrendingUp, DollarSign, AlertCircle, ArrowRight, ArrowUpRight, ArrowDownRight, Wallet, ShieldCheck, Calendar, Briefcase, Target, FileSearch, } from "lucide-react";
    ...
    return (<div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-muted p-1 rounded-lg self-start">
          {["7days", "30days", "month", "90days", "year"].map((p) => (<Button key={p} variant={period === p ? "secondary" : "ghost"} size="sm" onClick={() => setPeriod(p)} className={cn("text-xs capitalize", period === p && "bg-background shadow-sm")}>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="hover:shadow-md transition-all">
          ...
          <CardContent>
            <div className="flex items-center text-xs">
              {revenueDelta >= 0 ? (<span className="text-green-600 flex items-center"><ArrowUpRight className="h-3 w-3 mr-1"/>+{revenueDelta.toFixed(1)}%</span>) : (<span className="text-red-600 flex items-center"><ArrowDownRight className="h-3 w-3 mr-1"/>{revenueDelta.toFixed(1)}%</span>)}
        ...
        <Card className="hover:shadow-md transition-all">
          ...
          <CardContent>
            <div className="flex items-center text-xs">
              {expenseDelta <= 0 ? (<span className="text-green-600 flex items-center"><ArrowDownRight className="h-3 w-3 mr-1"/>{expenseDelta.toFixed(1)}%</span>) : (<span className="text-red-600 flex items-center"><ArrowUpRight className="h-3 w-3 mr-1"/>+{expenseDelta.toFixed(1)}%</span>)}
              <span className="ml-1 text-muted-foreground">vs mês anterior</span>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Link to="/financial" search={{ type: 'receivable' }} className="block group">
          <Card className="hover:border-primary transition-colors cursor-pointer h-full">
            ...
            <CardContent>
              ...
              <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-primary" style={{ width: `${Math.min(100, (finance.revenue / ((finance.revenue + finance.receivables) || 1)) * 100)}%` }}/>
              </div>
        ...
        <Link to="/financial" search={{ type: 'payable' }} className="block group">
          <Card className="hover:border-primary transition-colors cursor-pointer h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <CardContent>
              ...
              <div className="mt-4 h-2 w-full bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-red-500" style={{ width: `${Math.min(100, (finance.expenses / ((finance.expenses + finance.payables) || 1)) * 100)}%` }}/>
              </div>
              <div className="mt-2 flex justify-between text-[10px] uppercase font-bold text-muted-foreground">
                <span>Pago</span>
                <span>Pendente</span>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {commercial.insurerRanking.length > 0 ? commercial.insurerRanking.map((item, idx) => (<div key={item.name} className="flex flex-col gap-1">
                <div className="flex justify-between text-sm">
                  <span className="font-medium truncate max-w-[150px]">{idx + 1}. {item.name}</span>
                  <span className="font-bold">{formatCurrency(item.value)}</span>
                </div>
                <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary" style={{ width: `${commercial.insurerRanking[0] ? (item.value / commercial.insurerRanking[0].value) * 100 : 0}%` }}/>
                </div>
              </div>)) : (<p className="text-center text-sm text-muted-foreground py-10">Sem movimentações no período</p>)}
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        ].map((r) => (<Link key={r.label} to="/renewals" className="p-4 rounded-xl border bg-card hover:bg-accent transition-colors text-center">
